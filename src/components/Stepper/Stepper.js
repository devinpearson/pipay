...
export default class Stepper extends Component {
  ...
  stepMoveAllowed (skipValidationExecution = false) {
    ...
    if (this.props.dontValidate) {
      ...
      if (skipValidationExecution) {
        ...
        // the user is using a higer order component (HOC) for validation (e.g react-validation-mixin), this wraps the Stepper steps as a HOC,
  ...
  getClassName (className, i) {
    let liClassName = `${this.state.navState.styles[i]}`
    ...
    return this.props.steps.map((s, i) => (
      <li className={`steps-segment ${this.getClassName('class', i)}`} onClick={(evt) => { this.jumpToStep(evt) }}
        ...
        <div className="steps-content">
          <p className="is-size-4 has-text-weight-light">{this.props.steps[i].name}</p>
        </div>
      </li>
    ))
  }
            ...
            <div className="column">
              {
                this.props.showSteps
                  ? <ul className="steps has-content-centered has-gaps">
                    {this.renderSteps()}
                  </ul>
                  : <span></span>
              }
